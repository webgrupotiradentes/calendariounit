...
export function MacrosTable({ macros, onEdit, onDelete }: MacrosTableProps) {
    if (macros.length === 0) {
        return (
            <div className="bg-muted/30 rounded-2xl p-8 text-center sm:p-12 border border-border">
                ...
                <p className="text-muted-foreground">Nenhum local macro encontrado</p>
            </div>
        );
    }
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-border">
                        {macros.map((macro) => (
                            <tr key={macro.id} className="hover:bg-muted/30 transition-colors group">
                                ...
                                <td className="px-6 py-4 hidden sm:table-cell text-muted-foreground">
                                    {format(new Date(macro.createdAt), "d 'de' MMM, yyyy", { locale: ptBR })}
                                </td>
